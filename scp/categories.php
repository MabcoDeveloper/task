...
if($category && $category->ht['creator_staff_id'] != $thisstaff->getId())
    $errors['err'] = sprintf(__('%s: Edit permission is denied!'), __('Category'));

if($_POST){
    switch(strtolower($_POST['do'])) {
        case 'update':
            $_POST['creator_staff_id'] = $thisstaff->getId();
                ...
                switch(strtolower($_POST['a'])) {
                    ...
                    case 'delete':
                        ...
                        foreach ($categories as $c) {
                            if($c && $c->ht['creator_staff_id'] != $thisstaff->getId()) {
                                $errors['err'] = sprintf(__('%s: Edit permission is denied!'), __('Category'));
                                continue;
                            }
...
if($category || ($_REQUEST['a'] && !strcasecmp($_REQUEST['a'], 'add'))) {
    if ($category && $errors['err'] == sprintf(__('%s: Edit permission is denied!'), __('Category')))
        $page='categories.inc.php';
    else
