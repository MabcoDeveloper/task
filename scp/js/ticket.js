...
// $(document).on('change', '#assign-to', function() {
//     if ($(this).find(":selected").val() === "0" || $(this).find(":selected").val().includes("t")) {
//         $("#sla-plan").hide();
//         $("#due-date").hide();

//         $('#sla-plan select option').prop('selected', false);
//         $('#sla-plan select option[value=""]').prop('selected', true);
	...
	$.ajax(
		'../ajax.php/form/help-topics/' + $PID, {
			...
			success: function(json) {
				if (json != '') {
					$('#child_help_topic').css('display', 'table-row');
