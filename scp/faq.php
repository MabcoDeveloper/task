            ...
            if(!$faq) {
                ...
                switch(strtolower($_POST['a'])) {
                    case 'edit':
                        ...
                        break;
                    case 'publish';
                        if ($faq && $faq->ht['creator_staff_id'] != $thisstaff->getId()) {
                            $errors['err'] = sprintf(__('%s: Edit permission is denied!'), __('FAQ'));
                            break;
                        }
...
if ($faq && $faq->getId()) {
    ...
    if ($_REQUEST['a'] == 'edit' && $thisstaff->hasPerm(FAQ::PERM_MANAGE)) {
        if ($faq && $faq->ht['creator_staff_id'] != $thisstaff->getId()) {
            $errors['err'] = sprintf(__('%s: Edit permission is denied!'), __('FAQ'));
            $inc = 'faq-view.inc.php';
        } else {
        ...
        return $faq->printPdf();
} elseif($_REQUEST['a'] == 'add' && $thisstaff->hasPerm(FAQ::PERM_MANAGE)) {
    $inc = 'faq.inc.php';
