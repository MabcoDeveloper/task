(function ($) {
    ...
    $(document).on('change', '.dp.input-medium.search-query.hasDatepicker', function() {
        if (location.href.includes('reports.php')) {
            var RecurringScheduleText = $(this).val();
            var RecurringScheduleDateTime = new Date(RecurringScheduleText);
            var MinimumStartDate = new Date('3/7/20');
            
            if (RecurringScheduleDateTime < MinimumStartDate) {
                alert('Minimum start date is "3/7/20"');
                $(this).val('3/7/20');
            }
        }
    });

    $(document).on('click', '.ui-datepicker-current.ui-state-default.ui-priority-secondary.ui-corner-all', function() {
        var NowDate = $.datepicker.formatDate('m/d/y', new Date());
        $('.dp.input-medium.search-query.hasDatepicker').val(NowDate);
    });
})(window.jQuery);
