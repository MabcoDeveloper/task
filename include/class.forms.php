...
class AssigneeField extends ChoiceField {
    ...
    function getChoices($verbose=false) {
        global $cfg;
        $sqlGethelpTopic="SELECT `name` FROM `ost_team` WHERE `team_id` IN (SELECT `team_id` FROM `ost_help_topic`)";
        $z = array();
        if (($sqlGethelpTopic_Res = db_query($sqlGethelpTopic)) && db_num_rows($sqlGethelpTopic_Res)) {
            while (list($TeamID) = db_fetch_row($sqlGethelpTopic_Res)) {
                array_push($z, $TeamID);
            }
        }
        if (!isset($this->_choices)) {
            ...
            if (($teams = Team::getActiveTeams()))
                ...
                if (($key = array_search($name,$z)) !== false) {
...
class AssignmentForm extends Form {
    ...
    function getFields() {
        ...
        $fields = array(
            'department' => new DepartmentField(array(
                'id' => 4,
                'label' => __('Department'),
