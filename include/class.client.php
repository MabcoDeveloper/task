...
class EndUser extends BaseAuthenticatedUser {
    ...
    function getNumRefusedTickets($forMyOrg=false) {
        $sql="SELECT `ost_ticket`.`ticket_id` FROM `ost_ticket` WHERE `status_id`=7 AND (`user_id`=".$this->getId()." OR `to_user_id`=".$this->getId().")";
        if ($sql_Res = db_query($sql)) {
            $CCTasksCount = db_affected_rows($sql_Res);
