...
implements AuthenticatedUser, EmailContact, TemplateVariable, Searchable {
    ...
    function getNumCCTasks() {
        $GetCCTasksQ = "SELECT `id` FROM `ost_task` WHERE  `closed` IS NULL AND (`collab_1` = " . $this->getId() . " OR `collab_2` = " .$this->getId() . " OR `collab_3` = " . $this->getId()."  OR `collab_team` IN(" . implode(",", db_input($this->getTeams())) . "))";

                if ($GetCCTasks_Res = db_query($GetCCTasksQ)) {
                    $CCTasksCount = db_affected_rows($GetCCTasks_Res);
                    // echo "<script>$('#subnav6').html('CC ($CCTasksCount)');</script>";
                }
        return ($CCTasksCount > 0) ? $CCTasksCount : 0;
    }

    function getNumTodayClosed() {
        $sql1='SELECT DISTINCT `staff_id` FROM `ost_staff` WHERE `dept_id` IN(' . implode(',', db_input($this->getDepts())) . ') ';
                        ...
                        if (!$this->showAssignedOnly() && ($depts = $this->getDepts())) //Staff with limited access just see Assigned tasks.
                            $where[] = 'task.dept_id IN(' . implode(',', db_input($depts)) . ') ';

                        $where = implode(' OR ', $where);
                        if ($where) $where = 'AND ( ' . $where . ' ) ';
        $GetCCTasksQ = "SELECT task.`id` As iid ,task.`number` AS numii,task.`created` AS cc,`ost_task__cdata`.`title`  AS tit,`ost_department`.`name` AS depp ,CONCAT(from_id.`firstname`, ' ', from_id.`lastname`) AS ff ,CONCAT(to_id.`firstname`, ' ', to_id.`lastname`) AS To_name ,task.`updated` As dd " 
    ...
    function getNumAFMS() {
        ...
        $GetCCTasksQ = "SELECT task.`id`,task.`number`,task.`created`,`ost_task__cdata`.`title` ,`ost_department`.`name` ,CONCAT(from_id.`firstname`, ' ', from_id.`lastname`),CONCAT(to_id.`firstname`, ' ', to_id.`lastname`),task.`updated`,`ost_team`.`name`  " 
        . "FROM " . TASK_TABLE . " task "
        ."INNER JOIN `ost_task__cdata` ON `ost_task__cdata`.`task_id`=task.`id`  "
        ."INNER JOIN `ost_staff` as from_id ON from_id.`staff_id`=task.`assignor_id` " 
        ."LEFT JOIN `ost_staff` as to_id ON to_id.`staff_id`=task.`staff_id` "
        ...
        ." WHERE `ost_agent_users_tickets`.`staff_id` =".$this->getId()." AND `ost_thread`.`object_type`='A'  AND  task.`closed` IS NULL ";
    ...
    function getIfextended() {
        $GetCCTasksQ = "SELECT `ost_staff_dept_access`.`dept_id`  FROM `ost_staff_dept_access` INNER JOIN `ost_staff` ON `ost_staff`.`staff_id`=`ost_staff_dept_access`.`staff_id` WHERE `ost_staff_dept_access`.`dept_id`<> `ost_staff`.`dept_id` AND `ost_staff`.`staff_id`=" . $this->getId() ;
    ...
    static function GetStaffNameByID($id) {
        $sql = 'select CONCAT(firstname, " ", lastname) from ' . STAFF_TABLE
                . ' WHERE staff_id = ' . $id;
                $res = db_query($sql);

        return db_fetch_row($res)[0];
    }
    ...
    function update($vars, &$errors) {
        ...
        //Update can make general informs
        if(isset($vars['ongeneralform'])){
            $_SESSION['var']=1;
            $UpdateFlag= "UPDATE `ost_staff` SET `Flag_general_forms` = 1 WHERE `staff_id` = ".$this->staff_id;
            db_query($UpdateFlag);
        }
        else{
            $_SESSION['var']=0;
            $UpdateFlag= "UPDATE `ost_staff` SET `Flag_general_forms` = 0 WHERE `staff_id` = ".$this->staff_id;
            db_query($UpdateFlag);
        }

        //Update can close
        if(isset($vars['oncanclose'])){
            // $_SESSION['var']=1;
            $UpdateFlag= "UPDATE `ost_staff` SET `Flag_can_close` = 1 WHERE `staff_id` = ".$this->staff_id;
            db_query($UpdateFlag);
        }
        else{
            // $_SESSION['var']=0;
            $UpdateFlag= "UPDATE `ost_staff` SET `Flag_can_close` = 0 WHERE `staff_id` = ".$this->staff_id;
            db_query($UpdateFlag);
        }
        ...
        $this->isreportsadmin = $vars['isreportsadmin'];
