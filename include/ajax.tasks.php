    ...
    function add($tid=0, $vars=array()) {
        ...
        if ($_POST) {
            ...
            if ($isvalid) {
              ...
              if ($vars['internal_formdata'] && $vars['internal_formdata']['is_recurring']) {
                if (date('H') >= 12)
                    $info['error'] = sprintf('%s - %s', __('Error adding task'), __('You can\'t create recurring task after 2 PM!'));
              } else {
        ...
        try{
            if(isset($_SESSION['Child_task'])){
                $query ="UPDATE  `ost_task`  SET  `parent_task_id` = ".$_SESSION['Child_task']." , `child_task_id` = ".$task->getId()." WHERE  `ost_task`.`id` = ".$task->getId();
                db_query($query);
                echo "<script type='text/javascript'>
                console.log(".$_SESSION['Child_task'].");
...
function addsub($tid=0, $vars=array()) {
    global $thisstaff;
    ...
    if ($tid) {
        ...
        $originalTask = Task::lookup($tid);
        // session_start();
        // $_SESSION['Parent_task']=$originalTask->getId();
    }
    else
      unset($_SESSION[':form-data']);
    ...
    function preview($tid) {
        ...
        if(isset($_SESSION['Showteam'])){
            include STAFFINC_DIR . 'templates/team-preview.tmpl.php';
        }
        else{
    ...
    function assign($tid, $target=null) {
        ...
        // if (!$task->checkStaffPerm($thisstaff, Task::PERM_ASSIGN) || !($form=$task->getAssignmentForm($_POST, array('target' => $target))))
        //     Http::response(403, __('Permission denied'));
        $form=$task->getAssignmentForm($_POST, array('target' => $target));
        $errors = array();
        $info = array(
            ':title' => sprintf(__('Task #%s: %s'),
                $task->getNumber(),
        ...
        if ($_POST && $form->isValid()) {
            if ($task->assign($form, $errors, false)) {
                $_SESSION['::sysmsgs']['msg'] = sprintf(
                        __('%s successfully'),
                        sprintf(
                            __('%s assigned to %s'),
