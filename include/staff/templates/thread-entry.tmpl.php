...
<?php
$new_date = date("Y-m-d H:i:s", strtotime('+3 hours', strtotime($entry->created)));
date_default_timezone_set("Asia/Damascus");
        echo sprintf(__('<b>%s</b> posted %s'), $name,
            sprintf('<a name="entry-%d" href="#entry-%1$s"><time %s
                ...
                date(DateTime::W3C, Misc::db2gmtime($new_date)),
                Format::daydatetime($new_date),
                $timeFormat ? $timeFormat($new_date) : Format::datetime($new_date)
            )
        ); ?>
