...
<form action="tickets.php?a=open" method="post" class="save" enctype="multipart/form-data">
	...
	<table class="form_table fixed" width="1000" border="0" cellspacing="0" cellpadding="2">
		<thead>
		...
		<tbody>
			<tr>
				<th colspan="2">
					<em><strong><?php echo __('User, Organization and Collaborators'); ?></strong>: </em>
					<div class="error"><?php echo $errors['user']; ?></div>
				</th>
			</tr>
			<tr>
				<td>
					<table class="form_table" width="1000" border="0" cellspacing="0" cellpadding="2">
						<?php
						if ($user) { ?>
							<tr>
                      ...
                      "><i class="icon-user"></i>
										...
										<a class="inline button" style="overflow:inherit" href="#" onclick="javascript:
                    ...
                    "><i class="icon-retweet"></i> <?php echo __('Change'); ?></a>
						...
						?>
							<tr id="userRow">
								...
								<td>
									<span>
										<select class="userSelection" name="name" id="user-name" data-placeholder="<?php echo __('Select User'); ?>">
										</select>
									</span>
								...
								<div>
									<input type="hidden" size=45 name="email" id="user-email" class="attached" placeholder="<?php echo __('User Email'); ?>" autocomplete="off" autocorrect="off" value="<?php echo $info['email']; ?>" />
								</div>
							</tr>
						<?php
						} ?>
						<tr id="ccRow">
							...
							<td>
								<span>
									<select class="collabSelections" name="ccs[]" id="cc_users_open" multiple="multiple" ref="tags" data-placeholder="<?php echo __('Select Contacts'); ?>">
									</select>
								</span>
						...
						<tr id="organizationRow">
							<td width="160"><?php echo __('Organization'); ?>:</td>
							<td>
								<span>
									<select name="users_organization" id="organization_users_open" data-placeholder="<?php echo __('Select Organization'); ?>" style="max-width: unset;width: 34.5em;padding: 5px;height: unset;">
										<option value="" selected="selected">&mdash; <?php echo __('Select an Organization'); ?> &mdash;</option>
										<?php
		...
		<tbody>
			...
			<tr>
				<td width="160" class="required">
					<?php echo __('Parent Help Topic'); ?>:
				</td>
				<td>
					<select name="topicId" onchange="PullChildrenHelpTopics(this.value)">
						<?php
						if ($topics = Topic::getParentHelpTopics(false, false, true)) {
							if (count($topics) == 1)
			...
			<tr id="child_help_topic" style="display:none;">
				<td width="160" class="required">
					<?php echo __('Child Help Topic'); ?>:
				</td>
				<td>
			...
			<tr id="sla-plan" style="display: none;">
				<td width="160">
			...
			<tr id="due-date" style="display: none;">
				<td width="160">
		...
		<tbody>
					...
					<td colspan=2>
						...
						<textarea class="<?php if ($cfg->isRichTextEnabled()) echo 'richtext';
											?> draft draft-delete" data-signature="<?php
																					echo Format::htmlchars(Format::viewableImages($signature)); ?>" data-signature-field="signature" data-dept-field="deptId" placeholder="<?php echo __('Initial response for the ticket'); ?>" name="response" id="response" cols="21" rows="8" style="width:80%;" <?php
						...
						<table border="0" cellspacing="0" cellpadding="2" width="100%">
							...
							<tr>
								...
								<td>
									...
									<label><input type="radio" name="signature" value="dept" <?php echo ($info['signature'] == 'dept') ? 'checked="checked"' : ''; ?>> <?php echo sprintf(__('Department Signature (%s)'), __('if set')); ?></label>
								</td>
							</tr>
						</table>
					</td>
			...
			<tr>
				<td colspan=2>
					<textarea class="<?php if ($cfg->isRichTextEnabled()) echo 'richtext';
										?> draft draft-delete" placeholder="<?php echo __('Optional internal note (recommended on assignment)'); ?>" name="note" cols="21" rows="6" style="width:80%;" <?php
