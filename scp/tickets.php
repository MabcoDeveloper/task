...
if($_REQUEST['Mark']){
    // echo $_REQUEST['Mark'];
    $query ="INSERT INTO `ost_ticket_bookmark` (`staff_id`, `ticket_id`) VALUES (".$thisstaff->getId().",".$_REQUEST['Mark'].")";
    db_query($query);
...
if($_REQUEST['UnMark']){
    // echo $_REQUEST['Mark'];
    $query ="DELETE FROM `ost_ticket_bookmark` WHERE `id`=".$_REQUEST['UnMark'];
    // echo $query;
    db_query($query);
    ...
    if ($ticket && $ticket->getId()) {
        ...
        case 'reply':
            if (!$role || !$role->hasPerm(Ticket::PERM_REPLY)) {
                ...
                $vars = $_POST;
                if($vars['reply_status_id'] == 7){
                    $sqlupdate="UPDATE `ost_ticket` SET `status_id`=".$vars['reply_status_id']." ,  `closed`= NOW()  WHERE `ticket_id`=".$ticket->getId();
                    echo $sqlupdate;
                    db_query($sqlupdate);
                }
                ...
                if (isset($vars['is_done']) && $vars['is_done'] != null) {
                    $CurrentStep = $ticket->ht['current_step'];
                    $ticket->logEvent("Done");

                    if ($CurrentStep > 0) {
                        if ($ticket->IsLastStep()) {
                            $vars['reply_status_id'] = 3; // Set ticket status as closed.
                            $ticket->SetStepAsFinished(); // Set ticket current step number to -1
                        } else {
                            $ticket->MoveToNextStep();
                        }
                    }
                }
                 //Yaseen
                 if (isset($vars['Privious_step']) && $vars['Privious_step'] != null) {
                    $CurrentStep = $ticket->ht['current_step'];
                    if ($CurrentStep > 1) {
                        $sql02 = "UPDATE ost_ticket set current_step = current_step - 1 where ticket_id=" . $ticket->getid() . "";
                        db_query($sql02);
                    }
                }
            ...
            if (!$errors && ($response = $ticket->postReply($vars, $errors, $alert))) {
                ...
                Ticket::SendPushNotification($ticket->ht['ticket_id'], $vars['response'], $ticket->ht['user_id']);
...
// Add Closed By System Tab
$nav->addSubMenu(array('desc'=>__('Closed By System'),
                        'title'=> __('Closed By System'),
                        'href'=>'tickets.php?a=cbs',
                        'iconclass'=>'Ticket',
                        'id' => 'colsed-by-system'),
                     ($_REQUEST['a']=='Closed By System'));
...
$nav->addSubMenu(array('desc'=>__(' From My Showrooms'),
                           'title'=> __('Tickets  From My Showrooms'),
                           'href'=>'tickets.php?a=afms',
                           'iconclass'=>'Ticket',
                           'id' => 'assigned-my-staff'),
                        ($_REQUEST['a']==' From My Staff'));

//book mark ticket
$nav->addSubMenu(array('desc'=>__('BookMark Ticket'),
                           'title'=> __('BookMark Ticket'),
                           'href'=>'tickets.php?a=BookMark',
                           'iconclass'=>'Ticket',
                           'id' => 'BookMark'),
                        ($_REQUEST['a']=='BookMark'));
...
if($ticket) {
    ...
    if ($_REQUEST['a']=='open' && $thisstaff->hasPerm(Ticket::PERM_CREATE, false))
