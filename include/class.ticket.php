...
implements RestrictedAccess, Threadable, Searchable
{
    ...
    function checkStaffPerm($staff, $perm = null)
    {
        ...
        if ($this->CheckAgentUsersTicket($staff->getId(), $this->getId()))
            return true;
    ...
    function CheckAgentUsersTicket($StaffID, $TicketID)
    {
        $CurrentStaffID = $StaffID;
        $CurrentTicketID = $TicketID;

        $GetUsersStaffTicketsQ = "SELECT `ticket_id` FROM `ost_ticket` INNER JOIN `ost_agent_users_tickets` ON `ost_ticket`.`user_id` = `ost_agent_users_tickets`.`user_id` INNER JOIN `ost_user` ON `ost_user`.`id` = `ost_ticket`.`user_id` LEFT JOIN `ost_staff` ON `ost_ticket`.`staff_id` = `ost_staff`.`staff_id` WHERE `ost_agent_users_tickets`.`staff_id` = $CurrentStaffID AND `ticket_id` = $CurrentTicketID;";
    ...
    function updateEstDueDate($clearOverdue = true, $IsScheduled = 0, $NewEstDueDate = null)
    {
        $DueDate = $this->getEstDueDate();

        if ($IsScheduled == 1 && $NewEstDueDate != null) {
            $this->est_duedate = $NewEstDueDate;
        } else {
            if (date('H') + 2 >= 17) {
                $dt = new DateTime($this->getCreateDate());
                $this->est_duedate = $dt->add(new DateInterval('PT' . 17 . 'H'))->format('Y-m-d H:i:s');
            } else {
     ...
     * setStatusId
     *
     * Forcedly set the ticket status ID to the received status ID. No
    ...
    static function SendPushNotification($TicketID, $Body, $ToUserID = 0, $ToStaffID = 0)
    {
        global $thisstaff, $thisclient, $FirebaseAPI_Key;

        if ($ToUserID > 0) {
            $UserToken = "";
            $GetFCM_TokenQ = "SELECT `fcm_token` FROM `ost_user` WHERE `id` = $ToUserID";

            if (($GetFCM_Token_Res = db_query($GetFCM_TokenQ)) && db_num_rows($GetFCM_Token_Res)) {
                $UserToken = db_fetch_row($GetFCM_Token_Res)[0];
            }
            ...
            if ($UserToken !== "") {
                ...
                if ($thisclient) {
                    $CurrentUserID = $thisclient->getId();
                    $GetUsernameQ = "SELECT `name` FROM `ost_user` WHERE `id` = $CurrentUserID";
                } else {
                    $CurrentUserID = $ToUserID;
                    $GetUsernameQ = "SELECT `name` FROM `ost_user` 
                                     INNER JOIN `ost_ticket` ON `ost_user`.`id` = `ost_ticket`.`user_id`
                                     WHERE `ticket_id` = $TicketID";
                }

                if (($GetUsername_Res = db_query($GetUsernameQ)) && db_num_rows($GetUsername_Res)) {
                    $Username = db_fetch_row($GetUsername_Res)[0];
                }
                ...
                curl_setopt_array($cURL, array(
                    ...
                    CURLOPT_POSTFIELDS => "{\r\n   \"to\": \"$UserToken\",\r\n   \"content_available\": false,\r\n   \"data\": {\r\n        \"notification\": {\r\n\t\t    \"title\": \"$Username\",\r\n\t\t    \"body\": \"$Body\"\r\n    \t},\r\n\t    \"click_action\": \"/task/tickets.php?id=$TicketID\"\r\n    }\r\n}",
            ...
            $StaffToken = "";
            $GetFCM_TokenQ = "SELECT `fcm_token` FROM `ost_staff` WHERE `staff_id` = $ToStaffID";

            if (($GetFCM_Token_Res = db_query($GetFCM_TokenQ)) && db_num_rows($GetFCM_Token_Res)) {
                $StaffToken = db_fetch_row($GetFCM_Token_Res)[0];
            }

            if ($StaffToken != null && $StaffToken !== "") {
                $Title = "MABCO";
                ...
                if ($thisstaff) {
                    ...
                    if (($GetUserName_Res = db_query($GetUserNameQ)) && db_num_rows($GetUserName_Res)) {
                        $Title = db_fetch_row($GetUserName_Res)[0];
                    }
                }
                ...
                curl_setopt_array($cURL, array(
                    ...
                    CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
                    CURLOPT_CUSTOMREQUEST => "POST",
                    CURLOPT_POSTFIELDS => "{\r\n   \"to\": \"$StaffToken\",\r\n   \"content_available\": false,\r\n   \"data\": {\r\n        \"notification\": {\r\n\t\t    \"title\": \"$Title\",\r\n\t\t    \"body\": \"$Body\"\r\n    \t},\r\n\t    \"click_action\": \"/task/scp/tickets.php?id=$TicketID\"\r\n    }\r\n}",
                    CURLOPT_HTTPHEADER => array(
                        "Authorization: key=$FirebaseAPI_Key",
                        "Content-Type: application/json"
                    ),
    ...
    static function SendPushNotificationGen( $Body, $ToUserID = 0, $ToStaffID = 0)

    {
        global $thisstaff, $thisclient, $FirebaseAPI_Key;
        $Title ="General Informs From: ";
        if ($ToUserID > 0) {
            $UserToken = "";
            $GetFCM_TokenQ = "SELECT `fcm_token` FROM `ost_user` WHERE `id` = $ToUserID";
            ...
            if ($UserToken !== "") {
                ...
                curl_setopt_array($cURL, array(
                    ...
                    CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
                    CURLOPT_CUSTOMREQUEST => "POST",
                    CURLOPT_POSTFIELDS => "{\r\n   \"to\": \"$UserToken\",\r\n   \"content_available\": false,\r\n   \"data\": {\r\n        \"notification\": {\r\n\t\t    \"title\": \"$Title\",\r\n\t\t    \"body\": \"$Body\"\r\n    \t},\r\n\t    \"click_action\": \"/task/kb/index.php?link=gf\"\r\n    }\r\n}",
                    CURLOPT_HTTPHEADER => array(
                        "Authorization: key=$FirebaseAPI_Key",
                        "Content-Type: application/json"
                    ),
            ...
            if ($StaffToken != null && $StaffToken !== "") {
                ...
                curl_setopt_array($cURL, array(
                    ...
                    CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
                    CURLOPT_CUSTOMREQUEST => "POST",
                    CURLOPT_POSTFIELDS => "{\r\n   \"to\": \"$StaffToken\",\r\n   \"content_available\": false,\r\n   \"data\": {\r\n        \"notification\": {\r\n\t\t    \"title\": \"$Title\",\r\n\t\t    \"body\": \"$Body\"\r\n    \t},\r\n\t    \"click_action\": \"/task/scp/kb.php?link=gf\"\r\n    }\r\n}",
                    CURLOPT_HTTPHEADER => array(
                        "Authorization: key=$FirebaseAPI_Key",
                        "Content-Type: application/json"
                    ),
...
//send notific messagge 
static function SendPushNotificationMsg( $Body, $ToUserID = 0, $ToStaffID = 0,$MesssageId,$Title_)
        ...
        if ($ToUserID > 0) {
            ...
            if ($UserToken !== "") {
                ...
                curl_setopt_array($cURL, array(
                    ...
                    CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
                    CURLOPT_CUSTOMREQUEST => "POST",
                    CURLOPT_POSTFIELDS => "{\r\n   \"to\": \"$UserToken\",\r\n   \"content_available\": false,\r\n   \"data\": {\r\n        \"notification\": {\r\n\t\t    \"title\": \"$Title\",\r\n\t\t    \"body\": \"$Body\"\r\n    \t},\r\n\t    \"click_action\": \"/task/masseges.php?id=$MesssageId\"\r\n    }\r\n}",
                    CURLOPT_HTTPHEADER => array(
                        "Authorization: key=$FirebaseAPI_Key",
                        "Content-Type: application/json"
                    ),
            ...
            if ($StaffToken != null && $StaffToken !== "") {
                ...
                curl_setopt_array($cURL, array(
                    ...
                    CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
                    CURLOPT_CUSTOMREQUEST => "POST",
                    CURLOPT_POSTFIELDS => "{\r\n   \"to\": \"$StaffToken\",\r\n   \"content_available\": false,\r\n   \"data\": {\r\n        \"notification\": {\r\n\t\t    \"title\": \"$Title\",\r\n\t\t    \"body\": \"$Body\"\r\n    \t},\r\n\t    \"click_action\": \"/task/scp/masseges.php?id=$MesssageId\"\r\n    }\r\n}",
                    CURLOPT_HTTPHEADER => array(
                        "Authorization: key=$FirebaseAPI_Key",
                        "Content-Type: application/json"
                    ),
        ...
        switch ($status->getState()) {
            case 'closed':
                ...
                if ($set_closing_agent) {
                    ...
                    $Body = "Ticket '{$this->getSubject()}' closed by {$thisstaff->getName()}";
                } else {
                    $SystemStaff = Staff::GetStaffById(86);
                    $this->staff = $SystemStaff;

                    $ecb = function ($t) use ($status) {
                        $t->logEvent('closed', array('status' => array($status->getId(), $status->getName())), null, null, true);
                        $t->deleteDrafts();
                    };

                    $Body = "Ticket '{$this->getSubject()}' closed by SYSTEM due to 7 days opening";
                }

                $UserID = $this->getOwnerId();
                Ticket::SendPushNotification($TicketID, $Body, $UserID, 0);
                break;
            case 'open':
    ...
    function postReply($vars, &$errors, $alert = true, $claim = true)
    {
        ...
        if ($recipients instanceof MailingList)
            $vars['thread_entry_recipients'] = $recipients->getEmailAddresses();
        $vars['object_type_new'] = ObjectModel::OBJECT_TYPE_TICKET;
    ...
    function logEvent($state, $data = null, $user = null, $annul = null, $is_by_system = false)
    {
        $this->getThread()->getEvents()->log($this, $state, $data, $user, $annul, $is_by_system);
    }
    ...
    function IsLastStep()
    {
        $CurrentTicketHelpTopicID = $this->getTopicId();
        $CurrentTicketStepNum = $this->getCurrentStep();

        $CheckTicketFlowQ = "SELECT `step_number` FROM `ost_help_topic_flow` WHERE `help_topic_id` = $CurrentTicketHelpTopicID AND `step_number` = $CurrentTicketStepNum + 1 ORDER BY `id` LIMIT 1";

        if (($CheckTicketFlow_Res = db_query($CheckTicketFlowQ)) && db_num_rows($CheckTicketFlow_Res)) {
            return false;
        }
    ...
    {
        $CurrentTicketID = $this->getId();
        $GetUsersStaffTicketsQ = "UPDATE `ost_ticket` SET `current_step` = `current_step` + 1 WHERE `ticket_id` = $CurrentTicketID;";

        if (($GetUsersStaffTickets_Res = db_query($GetUsersStaffTicketsQ)) && db_affected_rows($GetUsersStaffTickets_Res)) {
            return true;
        }
    ...
    function SetStepAsFinished()
    {
        $CurrentTicketID = $this->getId();
        $GetUsersStaffTicketsQ = "UPDATE `ost_ticket` SET `current_step` = -1 WHERE `ticket_id` = $CurrentTicketID;";
    ...
    static function GetTicketById($TicketID)
    {
        if (!$TicketID)
            return null;

        $query = static::objects()
            ->filter(array('ticket_id' => $TicketID));
    ...
    static function GetCreatedDateById($TicketId)
    {
        $TicketCreatedDateQ = "SELECT `created` FROM `ost_ticket` WHERE `ticket_id` = $TicketId";

        if (($TicketCreatedDateRes = db_query($TicketCreatedDateQ)) && db_num_rows($TicketCreatedDateRes)) {
            return db_fetch_row($TicketCreatedDateRes)[0];
        }

        return false;
    }

    static function CheckLastStep($HelpTopicID, $TicketStepNum)
    {
        $TicketHelpTopicID = $HelpTopicID;

        $CheckTicketFlowQ = "SELECT `step_number` FROM `ost_help_topic_flow` WHERE `help_topic_id` = $TicketHelpTopicID AND `step_number` = $TicketStepNum + 1 ORDER BY `id` LIMIT 1";
    ...
    static function GetClosedDate($TicketID)
    {
        $CheckTicketFlowQ = "SELECT `closed` FROM `ost_ticket` WHERE `ticket_id` = $TicketID;";

        if (($CheckTicketFlow_Res = db_query($CheckTicketFlowQ)) && db_num_rows($CheckTicketFlow_Res)) {
            return db_fetch_row($CheckTicketFlow_Res)[0];
        }
    ...
    static function create($vars, &$errors, $origin, $autorespond = true, $alertstaff = true)
    {
        ...
        // Check if an Organization of users is chosen and convert it to CCs
        if ($UsersOrganizationID = $vars['users_organization']) {
            $GetOrganizationUsers_Q  = "SELECT `user_id` 
            FROM `ost_organization__teams` 
            WHERE `org_id` = $UsersOrganizationID";

            if (($GetOrganizationUsers_Res = db_query($GetOrganizationUsers_Q)) && db_num_rows($GetOrganizationUsers_Res)) {
                if (!is_array($vars['ccs']))
                    $vars['ccs'] = [];

                while (list($UserID) = db_fetch_row($GetOrganizationUsers_Res)) {
                    array_push($vars['ccs'], $UserID);
                }
            }
        }
        ...
        if (isset($vars['ccs']) && count($vars['ccs'])) {
            ...
            $GetUsers_Q  = "SELECT `ost_thread_collaborator`.`user_id` 
                            FROM `ost_thread` 
                            INNER JOIN `ost_thread_collaborator` ON `ost_thread`.`id` = `ost_thread_collaborator`.`thread_id` 
                            INNER JOIN `ost_ticket` ON `ost_ticket`.`ticket_id` = `ost_thread`.`object_id` 
                            WHERE `ost_thread`.`object_type` = 'T' AND `ost_ticket`.`ticket_id` = $TicketID";

            if (($GetUsers_Res = db_query($GetUsers_Q)) && db_num_rows($GetUsers_Res)) {
                while (list($UserID) = db_fetch_row($GetUsers_Res)) {
                    Ticket::SendPushNotification($TicketID, $Body, $UserID, 0);
                }
            }
        }
        ...
        if (array_key_exists("scheduled", $vars) && array_key_exists("new_est_duedate", $vars)) {
            $dt = new DateTime($vars['new_est_duedate']);
            $ticket->duedate = $dt->format('Y-m-d H:i:s');
            $ticket->updateEstDueDate(true, $vars['scheduled'], $vars['new_est_duedate']);
        } else {
