...
//  }
                                                        ...
                                                        if (isset($_POST["str2"])) {
                                                            ...
                                                            if ((($staffID != "Null" && $$departmentid != "Null")  ||   ($teamId != "Null" && $$departmentid != "Null") || $user != "Null")) {
                                                                $sql = 'INSERT INTO `ost_massage` ( `number`, `dept_id`, `staff_id`, `assignor_id`, `team_id`,`collab_1`,`collab_2`,`collab_3`,`created`,`user_id`,`title`,`is_private`,`From_user`,`TheTitle`) VALUES 
                                    ("' . ((int)$newnumber[0] + 1) . '",' . $departmentid . ',' . $staffID . ',' . "Null" . ',' . $teamId . ',' . $cc1 . ',' . $cc2 . ',' . $cc3 . ',Now(),' . $user . ',"' . $_SESSION["Subject"] . '",' . $gender . ',' . $thisclient->getId() . ',"' . $_SESSION["TT"]. '")';
                                                                // echo $sql;
        ...
        <div id="msg_error"><?php echo $errors['err']; ?></div><?php
                                                        ...
                                                        if (isset($_POST["Submit1"])) {
                                                            ...
                                                            if (!empty($_POST["replytext"])) {
                                                                $sql = 'INSERT INTO `ost_massage` ( `number`, `dept_id`, `staff_id`, `assignor_id`, `team_id`,`collab_1`,`collab_2`,`collab_3`,`created`,`title`,`reply`,`user_id`,`From_user`,`TheTitle`) VALUES 
    ("' . ((int)$newnumber[0] + 1) . '",' . $_SESSION["Dep"] . ',' . $_SESSION["FromStaffId"]  . ',' . "Null" . ',' . $_SESSION["Toteam"] . ',"' .  $_SESSION["cc1"]  . '","' .  $_SESSION["cc2"]  . '","' .  $_SESSION["cc3"]  . '",Now()'  . ',"' . str_replace('"', '', $_POST["replytext"]) . '","' . $_SESSION["Number"] . '",' . $_SESSION["userid"] . ',' . $thisclient->getId() .  ',"' . str_replace('"', '', $_POST["title"]) . '")';
                                                                // echo $sql;
                                                                ...
                                                                unset($_GET["reply"]);
                                                                $errors['err'] = __('something went wrong plese make sure you enter Reply message');
                                                                ?>
        <div id="msg_error"><?php echo $errors['err']; ?></div><?php
...
if (!isset($_GET["cm"]) && !isset($_GET["id"]) && !isset($_GET["forword"]) && !isset($_GET["readedmasseges"])) {
...
?>
    <div class="pull-right states">
        ...
        <a class="icon-mail-reply" href="?cm=cm">
        ...
        <a class="icon-mail-reply-all" href="?readedmasseges=readedmasseges">
            <?php echo _('Readed Message');
            ?>
        </a>
    ...
    <table style="margin-top:2em" id="ticketTable"  border="0" cellspacing="1" cellpadding="2" width="100%">
        <caption><?php echo  $results; ?></caption>
        <thead>
            <tr>
            <th width="20">
                <a href="masseges.php?sort=ID&order=<?php echo $negorder; ?><?php echo $qstr; ?>" title="Sort By Ticket ID"><?php echo __('#'); ?>&nbsp;<i class="icon-sort"></i></a>
                </th>
                <th width="100">
                <a href="masseges.php?sort=from_agent&order=<?php echo $negorder; ?><?php echo $qstr; ?>" title="Sort By 'From Agent'"> <?php echo __('From'); ?>&nbsp;<i class="icon-sort"></i></a>
                </th>
                <th width="120">
                <a href="masseges.php?sort=subject&order=<?php echo $negorder; ?><?php echo $qstr; ?>" title="Sort By Subject"><?php echo __('Subject'); ?>&nbsp;<i class="icon-sort"></i></a>
                </th>
                <th width="120">
                <a href="masseges.php?sort=date&order=<?php echo $negorder; ?><?php echo $qstr; ?>" title="Sort By Date"><?php echo __('Create Date'); ?>&nbsp;<i class="icon-sort"></i></a>
                </th>
                <th width="120">
                <a href="masseges.php?sort=to_agent&order=<?php echo $negorder; ?><?php echo $qstr; ?>" title="Sort By 'To Agent'"><?php echo __('To'); ?>&nbsp;<i class="icon-sort"></i></a>
                </th>
                <th width="110">
                <a href="masseges.php?sort=dept&order=<?php echo $negorder; ?><?php echo $qstr; ?>" title="Sort By Department"><?php echo __('Department'); ?>&nbsp;<i class="icon-sort"></i></a>
                </th>
                
               

                <th width="75">
                <a href="masseges.php?sort=team&order=<?php echo $negorder; ?><?php echo $qstr; ?>" title="Sort By Team"><?php echo __('Team'); ?>&nbsp;<i class="icon-sort"></i></a>
                </th>
                <th width="75">
                   <?php echo __('CC'); ?>
                </th>
                <th width="75">
                <a href="masseges.php?sort=to_user&order=<?php echo $negorder; ?><?php echo $qstr; ?>" title="Sort By 'To User'"><?php echo __('To User'); ?>&nbsp;<i class="icon-sort"></i></a>
                </th>
            </tr>
        </thead>
        <tbody>
            <?php
            $z = array();
            $sql="SELECT `msg_id` FROM `ost_read_messages` WHERE `user_id`=".$thisclient->getId();
            ...
            if(count($z)>0){
                $where1[] = ' AND  `ost_massage`.`id` NOT IN (SELECT `msg_id` FROM `ost_read_messages` WHERE `user_id`='.$thisclient->getId().') '; 
            }
...
$GetUsersStaffTicketsQ = "SELECT `ost_massage`.`id` ,`ost_massage`.`number` as os_number, `ost_massage`.`created` as message_created ,`ost_massage`.`TheTitle` as TT,`ost_department`.`name` as dep_name ,fromuser.`name` as FFname , CONCAT(t.`firstname`, ' ', t.`lastname`) as To_name ,`ost_team`.`name` as team_name, CONCAT(c1.`firstname`, ' ', c1.`lastname`),CONCAT(c2.`firstname`, ' ', c2.`lastname`), CONCAT(c3.`firstname`, ' ', c3.`lastname`),touser.`name` as Username,`ost_file_messages`.`file_id`
FROM `ost_massage` LEFT JOIN `ost_staff` as t ON t.`staff_id`=`ost_massage`.`staff_id` LEFT JOIN `ost_staff` as c1 ON c1.`staff_id`=`ost_massage`.`collab_1` LEFT JOIN `ost_staff` as c2 ON c2.`staff_id`=`ost_massage`.`collab_2` LEFT JOIN `ost_staff` as c3 ON c3.`staff_id`=`ost_massage`.`collab_3` LEFT JOIN `ost_team` ON `ost_team`.`team_id`=`ost_massage`.`team_id` LEFT JOIN`ost_department` ON `ost_department`.`id`=`ost_massage`.`dept_id` 
            INNER JOIN `ost_user` as fromuser ON fromuser.`id`=`ost_massage`.`From_user`
            ...
            WHERE (`ost_massage`.`From_user`=" . $thisclient->getId() ." AND `ost_massage`.`id` IN (SELECT `msg_id` FROM `ost_read_messages` WHERE `user_id`=". $thisclient->getId() ." )) OR
            (`ost_massage`.`user_id`=" . $thisclient->getId() . " OR `ost_massage`.`From_user`=" . $thisclient->getId() .") ". $where1[0]."
            UNION
            SELECT `ost_massage`.`id` ,`ost_massage`.`number` as os_number, `ost_massage`.`created` as message_created ,`ost_massage`.`TheTitle` as TT,`ost_department`.`name` as dep_name ,CONCAT(f.`firstname`, ' ', f.`lastname`) as FFname,CONCAT(t.`firstname`, ' ', t.`lastname`) as To_name ,`ost_team`.`name` as team_name, CONCAT(c1.`firstname`, ' ', c1.`lastname`),CONCAT(c2.`firstname`, ' ', c2.`lastname`), CONCAT(c3.`firstname`, ' ', c3.`lastname`),touser.`name` as Username,`ost_file_messages`.`file_id`
             FROM `ost_massage` INNER JOIN `ost_staff` as f ON f.`staff_id`=`ost_massage`.`assignor_id` LEFT JOIN `ost_staff` as t ON t.`staff_id`=`ost_massage`.`staff_id` LEFT JOIN `ost_staff` as c1 ON c1.`staff_id`=`ost_massage`.`collab_1` LEFT JOIN `ost_staff` as c2 ON c2.`staff_id`=`ost_massage`.`collab_2` LEFT JOIN `ost_staff` as c3 ON c3.`staff_id`=`ost_massage`.`collab_3` LEFT JOIN `ost_team` ON `ost_team`.`team_id`=`ost_massage`.`team_id` LEFT JOIN`ost_department` ON `ost_department`.`id`=`ost_massage`.`dept_id` 
            LEFT JOIN `ost_user` as touser ON touser.`id`=`ost_massage`.`user_id`
            LEFT JOIN  `ost_file_messages` ON `ost_file_messages`.`msg_id`= `ost_massage`.`id`
            LEFT JOIN `ost_file` ON `ost_file`.`id`=`ost_file_messages`.`file_id`
            WHERE (`ost_massage`.`From_user`=" . $thisclient->getId() ." AND `ost_massage`.`id` IN (SELECT `msg_id` FROM `ost_read_messages` WHERE `user_id`=". $thisclient->getId() ." )) OR
             (`ost_massage`.`user_id`=" . $thisclient->getId() . " OR `ost_massage`.`From_user`=" . $thisclient->getId().") ". $where1[0]."  ORDER BY ". $OrderBy;
            //  echo $GetUsersStaffTicketsQ;
                                                            if (($GetUsersStaffTickets_Res = db_query($GetUsersStaffTicketsQ)) && db_num_rows($GetUsersStaffTickets_Res)) {
                                                                while (list($ID, $Number, $Created, $Subject, $Dep, $FromStaff, $ToStaff, $team, $cc1, $cc2, $cc3, $touser,$fileID) = db_fetch_row($GetUsersStaffTickets_Res)) {
                                                                    if ($team == null) {
                                                                        $teamID = "no team";
                                                                    } else {
                                                                        $teamID = $team;
            ...
            ?>
                    <tr id="<?php echo $Number; ?>">
                        <td style='font-weight:bold'><?php echo $Number; ?></td>
                        <td style="font-weight:bold;"><span><?php echo $FromStaff; ?></span></td>
                        <?Php if($fileID != null ) {?>
...
if (($staffID != "Null" ||   $teamId != "Null" || $user != "Null") && $name != "Null" && $title!="Null") {
    $sql = 'INSERT INTO `ost_massage` ( `number`, `dept_id`, `staff_id`, `assignor_id`, `team_id`,`collab_1`,`collab_2`,`collab_3`,`created`,`user_id`,`title`,`is_private`,`From_user`, `TheTitle`) VALUES 
                                ("' . ((int)$newnumber[0] + 1) . '",' . $departmentid . ',' . $staffID . ',' . "Null" . ',' . $teamId . ',' . $cc1 . ',' . $cc2 . ',' . $cc3 . ',Now(),' . $user . ',"' . str_replace('"', '', $name) . '",' . $gender . ',' . $thisclient->getId() . ',"' . str_replace('"', '', $title) . '")';
    echo $sql;
    $GetStaffNameQ = "SELECT `name` FROM `ost_user` WHERE `id` = " . $thisclient->getId();
    ...
    if ($staffID != null) {
        $sqlgetmaxnumber = "SELECT MAX(`id`) FROM `ost_massage`";
        if (($sqlgetmaxnumber_Res = db_query($sqlgetmaxnumber)) && db_num_rows($sqlgetmaxnumber_Res)) {
...
if (isset($_GET["id"])) {
// echo "hellllllo";
$sqlisread="INSERT INTO `ost_read_messages` (`id`, `msg_id`, `staff_id`, `user_id`) VALUES (NULL, ".$_GET["id"].", NULL".", ".$thisclient->getId().")";
// echo $sqlisread;
    db_query($sqlisread);
?>
    <strong>
    ...
    <table id="ticketTable" cellspacing="0" cellpadding="0" width="100%" border="0">
        <tr>
          ...
          CONCAT(c3.`firstname`, ' ', c3.`lastname`),f.`staff_id`,t.`staff_id`,`ost_user`.`name`, `ost_user`.`id`,fromuser.`name`,fromuser.`id`
          ,`ost_file`.`id`,`ost_file`.`name`,`ost_attachment`.`id`,`ost_massage`.`TheTitle`
    ...
    <div id="task-form">
        ...
        <div id="new-task-form" style="display:block;">
            <form method="post" class="org" action="" id="formID">
                ...
                Is Private(Default Is Private):
                <!-- <input type="checkbox" name="yes" <?php //if (isset($gender) && $gender == "yes") echo "checked"; ?> value="yes">yes -->
                 <input type="radio" name="no" <?php if (isset($gender) && $gender == "no") echo "checked"; 
                                                    ?> value="no">no 
...
?>
    ...
    <table style="margin-top:2em" id="ticketTable"  border="0" cellspacing="1" cellpadding="2" width="100%">
        ...
        <thead>
            <tr>
            <th width="20">
                <a href="masseges.php?readedmasseges=readedmasseges&sort=ID&order=<?php echo $negorder; ?><?php echo $qstr; ?>" title="Sort By Ticket ID"><?php echo __('#'); ?>&nbsp;<i class="icon-sort"></i></a>
                </th>
                <th width="100">
                <a href="masseges.php?readedmasseges=readedmasseges&sort=from_agent&order=<?php echo $negorder; ?><?php echo $qstr; ?>" title="Sort By 'From Agent'"> <?php echo __('From'); ?>&nbsp;<i class="icon-sort"></i></a>
                </th>
                <th width="120">
                <a href="masseges.php?readedmasseges=readedmasseges&sort=subject&order=<?php echo $negorder; ?><?php echo $qstr; ?>" title="Sort By Subject"><?php echo __('Subject'); ?>&nbsp;<i class="icon-sort"></i></a>
                </th>
                <th width="120">
                <a href="masseges.php?readedmasseges=readedmasseges&sort=date&order=<?php echo $negorder; ?><?php echo $qstr; ?>" title="Sort By Date"><?php echo __('Create Date'); ?>&nbsp;<i class="icon-sort"></i></a>
                </th>
                <th width="120">
                <a href="masseges.php?readedmasseges=readedmasseges&sort=to_agent&order=<?php echo $negorder; ?><?php echo $qstr; ?>" title="Sort By 'To Agent'"><?php echo __('To'); ?>&nbsp;<i class="icon-sort"></i></a>
                </th>
                <th width="110">
                <a href="masseges.php?readedmasseges=readedmasseges&sort=dept&order=<?php echo $negorder; ?><?php echo $qstr; ?>" title="Sort By Department"><?php echo __('Department'); ?>&nbsp;<i class="icon-sort"></i></a>
                </th>
                
               

                <th width="75">
                <a href="masseges.php?readedmasseges=readedmasseges&sort=team&order=<?php echo $negorder; ?><?php echo $qstr; ?>" title="Sort By Team"><?php echo __('Team'); ?>&nbsp;<i class="icon-sort"></i></a>
                </th>
                <th width="75">
                   <?php echo __('CC'); ?>
                </th>
                <th width="75">
                <a href="masseges.php?readedmasseges=readedmasseges&sort=to_user&order=<?php echo $negorder; ?><?php echo $qstr; ?>" title="Sort By 'To User'"><?php echo __('To User'); ?>&nbsp;<i class="icon-sort"></i></a>
                </th>
            </tr>
        </thead>
        <tbody>
...
FROM `ost_massage` LEFT JOIN `ost_staff` as t ON t.`staff_id`=`ost_massage`.`staff_id` LEFT JOIN `ost_staff` as c1 ON c1.`staff_id`=`ost_massage`.`collab_1` LEFT JOIN `ost_staff` as c2 ON c2.`staff_id`=`ost_massage`.`collab_2` LEFT JOIN `ost_staff` as c3 ON c3.`staff_id`=`ost_massage`.`collab_3` LEFT JOIN `ost_team` ON `ost_team`.`team_id`=`ost_massage`.`team_id` LEFT JOIN`ost_department` ON `ost_department`.`id`=`ost_massage`.`dept_id` 
            ...
            WHERE `ost_read_messages`.`user_id`= " . $thisclient->getId() ." 
            UNION
            SELECT `ost_massage`.`id` ,`ost_massage`.`number` as os_number, `ost_massage`.`created` as message_created ,`ost_massage`.`TheTitle` as TT,`ost_department`.`name` as dep_name ,CONCAT(f.`firstname`, ' ', f.`lastname`) as FFname,CONCAT(t.`firstname`, ' ', t.`lastname`) as To_name ,`ost_team`.`name` as team_name, CONCAT(c1.`firstname`, ' ', c1.`lastname`),CONCAT(c2.`firstname`, ' ', c2.`lastname`), CONCAT(c3.`firstname`, ' ', c3.`lastname`),touser.`name` as Username,`ost_file_messages`.`file_id`
             FROM `ost_massage` INNER JOIN `ost_staff` as f ON f.`staff_id`=`ost_massage`.`assignor_id` LEFT JOIN `ost_staff` as t ON t.`staff_id`=`ost_massage`.`staff_id` LEFT JOIN `ost_staff` as c1 ON c1.`staff_id`=`ost_massage`.`collab_1` LEFT JOIN `ost_staff` as c2 ON c2.`staff_id`=`ost_massage`.`collab_2` LEFT JOIN `ost_staff` as c3 ON c3.`staff_id`=`ost_massage`.`collab_3` LEFT JOIN `ost_team` ON `ost_team`.`team_id`=`ost_massage`.`team_id` LEFT JOIN`ost_department` ON `ost_department`.`id`=`ost_massage`.`dept_id` 
            LEFT JOIN `ost_user` as touser ON touser.`id`=`ost_massage`.`user_id`
            ...
            LEFT JOIN `ost_file` ON `ost_file`.`id`=`ost_file_messages`.`file_id`
            LEFT JOIN `ost_read_messages` ON `ost_read_messages`.`msg_id`=`ost_massage`.`id`
            WHERE `ost_read_messages`.`user_id`= " . $thisclient->getId() ."  ORDER BY ". $OrderBy;
            //  echo $GetUsersStaffTicketsQ;
                                                            if (($GetUsersStaffTickets_Res = db_query($GetUsersStaffTicketsQ)) && db_num_rows($GetUsersStaffTickets_Res)) {
                                                                while (list($ID, $Number, $Created, $Subject, $Dep, $FromStaff, $ToStaff, $team, $cc1, $cc2, $cc3, $touser,$fileID) = db_fetch_row($GetUsersStaffTickets_Res)) {
                                                                    if ($team == null) {
