...
implements TemplateVariable, Searchable {
    ...
    static function getParentHelpTopics() {
        $GetRecurringTaskQ = "SELECT `topic_id`, `topic` FROM `ost_help_topic` WHERE `topic_pid` = 0 AND `flags` = 2;";
        $Topics = array();

        if(($GetRecurringTask_Res = db_query($GetRecurringTaskQ)) && db_num_rows($GetRecurringTask_Res)) {
            while (list($TopicID, $TopicTitle) = db_fetch_row($GetRecurringTask_Res)) {
    ...
    static function getChildrenForHelpTopic($topic_id) {
        $GetRecurringTaskQ = "SELECT `topic_id`, `topic` FROM `ost_help_topic` WHERE `topic_pid` = $topic_id AND `flags` = 2;";
        $Topics = array();
      ...
      if (!$names || $localize) {
          ...
          foreach ($topics as $id=>$info) {
              ...
              while (($pid = $info['pid']) && ($info = $topics[$info['pid']])) {
                  $name = sprintf('%s / %s', $localize_this($pid, $info['topic']), $name);
