...
implements Searchable {
    ...
    static function getLastThreadEntry($TicketID) {
        return ThreadEntry::getLastThreadEntry($TicketID);;
    }

    static function getLastThreadEntryTask($TicketID) {
        return ThreadEntry::getLastThreadEntryTask($TicketID);;
    }
...
implements TemplateVariable {
    ...
    */
    ...
    static function getLastThreadEntry($TicketID) {
        $LastThreadEntry_Q = '
        SELECT `ost_thread_entry`.`body`,`ost_thread_entry`.`created`  ,`ost_thread_entry`.`poster`
        FROM `ost_thread_entry` 
        INNER JOIN `ost_thread` ON `ost_thread_entry`.`thread_id` = `ost_thread`.`id` 
        WHERE `object_type` LIKE \'T\' AND `ost_thread`.`object_id` = ' . $TicketID . ' AND `ost_thread_entry`.`created` = 
        (SELECT MAX(`ost_thread_entry`.`created`) 
        FROM `ost_thread_entry` 
        INNER JOIN `ost_thread` ON `ost_thread_entry`.`thread_id` = `ost_thread`.`id` 
        WHERE `ost_thread`.`object_id` = ' . $TicketID . ' AND `object_type` LIKE \'T\');';
        
        if(($LastThreadEntry_Res = db_query($LastThreadEntry_Q)) && db_num_rows($LastThreadEntry_Res)) {
            $Res = db_fetch_row($LastThreadEntry_Res);
    ...
    static function getLastThreadEntryTask($TaskId) {
        $LastThreadEntry_Q = '
        SELECT `ost_thread_entry`.`body`,`ost_thread_entry`.`created`  ,`ost_thread_entry`.`poster`
        FROM `ost_thread_entry` 
        INNER JOIN `ost_thread` ON `ost_thread_entry`.`thread_id` = `ost_thread`.`id` 
        WHERE `object_type` LIKE \'A\' AND `ost_thread`.`object_id` = ' . $TaskId . ' AND `ost_thread_entry`.`created` = 
        (SELECT MAX(`ost_thread_entry`.`created`) 
        FROM `ost_thread_entry` 
        INNER JOIN `ost_thread` ON `ost_thread_entry`.`thread_id` = `ost_thread`.`id` 
        WHERE `ost_thread`.`object_id` = ' . $TaskId . ' AND `object_type` LIKE \'A\');';
...
class ThreadEvents extends InstrumentedList {
     ...
     *      this event
     */
    function log($object, $state, $data=null, $user=null, $annul=null, $is_by_system = false) {
        global $thisstaff, $thisclient;
...
class InProgressEvent extends ThreadEvent {
    static $icon = 'time';
    static $state = 'InProgress';

    function getDescription($mode=self::MODE_STAFF) {
        return $this->template(__('set as in progress by <b>{somebody}</b> {timestamp}'), $mode);
    }
}
...
implements TemplateVariable {
    ...
    function addResponse($vars, &$errors) {
        ...
        $vars['flags'] = 0;
        if($vars['object_type_new']=='A'){
            $query ="UPDATE  `ost_task`  SET  `updated` = NOW()  WHERE  `ost_task`.`id` = (SELECT `ost_task`.`id` FROM `ost_task`  INNER JOIN `ost_thread` ON `ost_task`.`id`=`ost_thread`.`object_id` WHERE `ost_thread`.`object_type`='A' AND `ost_thread`.`id`= ".$this->getId().")";

        }
        if($vars['object_type_new']=='T'){
            $query ="UPDATE  `ost_ticket`  SET  `lastupdate` = NOW()  WHERE  `ost_ticket`.`ticket_id` = (SELECT `ost_ticket`.`ticket_id` FROM `ost_ticket`  INNER JOIN `ost_thread` ON `ost_ticket`.`ticket_id`=`ost_thread`.`object_id` WHERE `ost_thread`.`object_type`='T' AND `ost_thread`.`id`= ".$this->getId().")";
