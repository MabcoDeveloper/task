      ...
      <tbody>
        ...
        <tr>
          <td colspan="2">
            ...
            <label class="checkbox" style="cursor:pointer;">
            <input type="checkbox" name="isreportsadmin" value="1"
              <?php echo ($staff->isreportsadmin) ? 'checked="checked"' : ''; ?> />
              <?php echo __('Reports Admin'); ?>
            </label>
            <label class="checkbox" style="cursor:pointer;">
            ...
            <?php 
            $GetGeneralForms="SELECT `Flag_general_forms` FROM `ost_staff` WHERE `staff_id`=".$staff->getId();
            // echo $GetGeneralForms;
            if (($GetGeneralForms_Res = db_query($GetGeneralForms)) && db_num_rows($GetGeneralForms_Res)) {
            while (list($CC1_) = db_fetch_row($GetGeneralForms_Res)) {
                $CC1= $CC1_;  
                }   
            }
            ...
            ?>
            <label class="checkbox" style="cursor:pointer;">
            <input type="checkbox" name="ongeneralform"
              <?php echo $CC1 ? 'checked="checked"' : ''; ?> />
              <?php echo __('Allow Create General Informs '); ?>
            </label>

            <?php 
            $GetCanClose="SELECT `Flag_can_close` FROM `ost_staff` WHERE `staff_id`=".$staff->getId();
            // echo $GetGeneralForms;
            if (($GetCanClose_Res = db_query($GetCanClose)) && db_num_rows($GetCanClose_Res)) {
            while (list($CanClose_) = db_fetch_row($GetCanClose_Res)) {
                $CanClose= $CanClose_;  
                }   
            }
            ...
            ?>
            <label class="checkbox" style="cursor:pointer;">
            <input type="checkbox" name="oncanclose"
              <?php echo $CanClose ? 'checked="checked"' : ''; ?> />
              <?php echo __('Permission Close Task '); ?>
            </label>
            <br/>
        </tr>
      </tbody>
...
<?php if(isset($_SESSION["staff_page_number"])){
?>
<button type="button"><a href="staff.php?sort=&order=&p=<?php echo $_SESSION["staff_page_number"]; ?>">Back</a></button>
<?php
}
else{
  ?>
  <button type="button"><a href="staff.php">Back</a></button>
  <?php
}