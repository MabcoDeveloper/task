...
if(isset($_GET["link"] ) ){
    ...
    $x = array();
    $y = array();
    $GetAllStaff = "SELECT `ost_general_informs`.`content` , CONCAT(`ost_staff`.`firstname`, ' ', `ost_staff`.`lastname`) ,`ost_general_informs`.`created_at` FROM `ost_general_informs` INNER JOIN `ost_staff` ON `ost_staff`.`staff_id`=`ost_general_informs`.`staff_id`  WHERE `ost_general_informs`.`Flag_agent`=1  OR  `ost_general_informs`.`staff_id`= ".$thisstaff->getId();
