...
class UsersAjaxAPI extends AjaxController {
    ...
    function setFCMToken() {
        ...
        $UserID = $thisclient->session->userID;
		...
		$UpdateFCM_TokenQ = "UPDATE `ost_user` SET `fcm_token` = '$Token' WHERE `id` = $CurrentUserID";

        if (($UpdateFCM_Token_Res = db_query($UpdateFCM_TokenQ)) && db_affected_rows($UpdateFCM_Token_Res)) {
		...
		if (($GetUserName_Res = db_query($GetUserNameQ)) && db_num_rows($GetUserName_Res)) {
			$Response['user_name'] = db_fetch_row($GetUserName_Res)[0];
        }
