...
function isAdmin(){
    $z=array();
    $sql="SELECT `ost_staff`.`staff_id` FROM `ost_staff` 
                    INNER JOIN `ost_team_member` ON `ost_team_member`.`staff_id`=`ost_staff`.`staff_id`
                    INNER JOIN `ost_team` ON `ost_team`.`team_id`=`ost_team_member`.`team_id`
                    WHERE `ost_team`.`team_id`=68";
if (($sql_Res = db_query($sql)) && db_num_rows($sql_Res)) {
    while (list($ID) = db_fetch_row($sql_Res)) {
        array_push($z,$ID);
    }}
    // echo $ID[0];
    if(in_array($this->staff->getId(), $z)){
        return 1;
    }
    else{
        return 0;
    ...
    function getTabs(){
        ...
        if(!$this->tabs) {
            ...
            if ($GetTeamId_=="154")
            {
                $this->tabs['tickets'] = array('desc'=>__('Tickets'),'href'=>'tickets.php?queue=20','title'=>__('Ticket Queue'));
            }
            else 
            {
            $this->tabs['tickets'] = array('desc'=>__('Tickets'),'href'=>'tickets.php?queue=5','title'=>__('Ticket Queue'));
            }
            $this->tabs["messages"] = array('desc'=>__('Messages'),'href'=>'masseges.php','title'=>__('Messages'), 'iconclass'=>'logs');

            $this->tabs["file"] = array('desc'=>__('File Management'),'href'=>'file_maneger.php','title'=>__('File Management'), 'iconclass'=>'logs');
            ...
            // if ($thisstaff->isManager()) {
                $this->tabs['reports'] = array(
                    'desc'=>__('Reports'),'href'=>'reports.php','title'=>__('Reports'), "class"=>"no-pjax"
                );
            // }
            $this->tabs['cal'] = array('desc'=>__('Calendar'),'href'=>'cal.php?cal=mine','title'=>__('Calendar'));
        ...
        foreach($this->getTabs() as $k=>$tab){
            ...
            switch(strtolower($k)){
                case 'tasks':
                    ...
                    break;
                case 'messages':
                    $subnav[]=array('desc'=>__('Messages'), 'href'=>'masseges.php', 'iconclass'=>'Ticket', 'droponly'=>true);
                    break;
                case 'dashboard':
                    // if ($staff->isManager()) {
                ...
                case 'kbase':
                    ...
                    $subnav[]=array('desc'=>__('General Informs'),'href'=>'kb.php?link=gf', 'urls'=>array('faq.php'), 'iconclass'=>'kb');

                   break;
                case 'reports':                    
                    $subnav[]=array('desc'=>__('Task Report'),'href'=>'reports.php?data=agentreport','iconclass'=>'teams');
                    if($staff->isManager()){
                    $subnav[]=array('desc'=>__('Task Total times spent '),'href'=>'reports.php?data=manegerreport','iconclass'=>'teams');    
                    }
                    if($this->isAdmin()==1){
                        
                        $subnav[]=array('desc'=>__('Task Admin Report '),'href'=>'reports.php?data=adminreport','iconclass'=>'teams');    
                        }

                    $subnav[]=array('desc'=>__('Agent Reports'),'href'=>'reports.php','iconclass'=>'teams');
                    break;
                    case 'cal':                    
                    $subnav[]=array('desc'=>__('My Calendar'),'href'=>'cal.php?cal=mine','iconclass'=>'faq-categories');
                        
                    $subnav[]=array('desc'=>__('Staff Calendar '),'href'=>'cal.php?cal=stafff','iconclass'=>'faq-categories');    
        ...
        foreach($this->getTabs() as $k=>$tab){
            ...
            switch(strtolower($k)){
                ...
                case 'manage':
                    ...
                    $subnav[]=array('desc'=>__('Ticket Filters'),'href'=>'filters.php', 'title'=>__('Ticket Filters'),'iconclass'=>'ticketFilters');
                ...
                case 'emails':
                    ...
                    $subnav[]=array('desc'=>__('Banlist'),'href'=>'banlist.php', 'title'=>__('Banned Emails'),'iconclass'=>'emailDiagnostic');
        ...
        if(!$this->navs){
          ...
          $OpenTasks=array();
            ...
            if($user && $user->isValid()) {
                ...
                WHERE (`ost_thread_collaborator`.`user_id`=".$user->getId()." OR `ost_thread_collaborator`.`team_id` IN(SELECT `team_id` FROM `ost_team_user_member` WHERE `user_id`=".$user->getId().") ) AND `ost_thread`.`object_type`='A' AND `ost_task`.`closed` IS NULL ";
                ...
                if(!$user->isGuest()) {
                    $navs['tasks']=array('desc'=>sprintf(__('Task (%d)'),$OpenTasks[0]),'href'=>sprintf('tickets.php?user_id=%d',$user->getId()),'title'=>__('Show all tasks'));
                    $navs['tickets']=array('desc'=>sprintf(__('Tickets (%d)'),$user->getNumTickets($user->canSeeOrgTickets())),
                                           'href'=>'tickets.php',
                                            'title'=>__('Show all tickets'));
                    $navs['mess']=array('desc'=>__('Messages'),'href'=>'masseges.php','title'=>'');
                    $navs['general_informs']=array('desc'=>__('General Informs'),'href'=>'kb/index.php?link=gf','title'=>'');
                    $navs["file"] = array('desc'=>__('File Management'),'href'=>'file_maneger.php','title'=>__('File Management'), 'iconclass'=>'logs');
