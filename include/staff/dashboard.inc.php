...
foreach ($groups as $g=>$desc) {
	if (Format::htmlchars($desc) === 'Agent - Help Topics') { ?>
...
<?php
    foreach ($data['data'] as $i=>$row) {
        ...
        foreach ($row as $j=>$td) {
            if ($j === 0) { 
                if ($data['columns'][0] === 'Agent') {
					$Staff = explode(' ', Format::htmlchars($td));
					$StaffFirstname = $Staff[0];
					$StaffLastname = $Staff[1];
					
					$StaffID = 0;
					$GetStaffID_Q = "SELECT `staff_id` FROM `ost_staff` WHERE `firstname` LIKE '$StaffFirstname' AND `lastname` LIKE '$StaffLastname';";
					
					if (($GetStaffID_Res = db_query($GetStaffID_Q)) && db_num_rows($GetStaffID_Res)) {
						$Res = db_fetch_row($GetStaffID_Res);
