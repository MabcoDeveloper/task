...
if ($team && $_REQUEST['a']!='add') {
    ...
    if(isset($_POST['custId'])){
      foreach($_SESSION['Users_names'] as $user_id){
        if(!in_array($user_id, $_POST['custId'])){
          
        
         $deletefile="DELETE FROM `ost_team_user_member` WHERE `team_id`=".$_REQUEST['id']." AND `user_id`=".$user_id;
   ...
   <table class="two-column table" width="100%">
    <tbody>
        ...
        <?php if (isset($_REQUEST['id'])){
          $_SESSION['Users_names'] = array();
                      echo "<br>";
                      $IDS=array();
                      $sql="SELECT `ost_team`.`team_id` FROM `ost_team` INNER JOIN `ost_team_user_member` ON `ost_team_user_member`.`team_id`=`ost_team`.`team_id`
                      GROUP BY `ost_team`.`team_id`";
                      
                      if (($sql_Res = db_query($sql)) && db_num_rows($sql_Res)) {
                        while (list($Name) = db_fetch_row($sql_Res)) {

                        array_push($IDS,$Name);
                        }}

                        if(in_array($_REQUEST['id'],$IDS)){
                          // echo $_REQUEST['id'];
                          $Use_Name=array();
                          $Use_Id=array();
                          $sql="SELECT `ost_team_user_member`.`user_id`, `ost_user`.`name` FROM `ost_team_user_member` INNER JOIN `ost_user` ON `ost_user`.`id`=`ost_team_user_member`.`user_id`
                          WHERE `ost_team_user_member`.`team_id`=".$_REQUEST['id']."  ORDER BY `ost_user`.`name`";
                          // echo $sql;
                          if (($sql_Res = db_query($sql)) && db_num_rows($sql_Res)) {
                            while (list($Id,$Name) = db_fetch_row($sql_Res)) {
                             
                        // echo $sql;
                            array_push($Use_Name,$Name);
                            array_push($Use_Id,$Id);
                            }}
                            $_SESSION['Users_names']=$Use_Id;
// print_r($Use_Id);
// <input type="hidden"  name="custId" value="<?php echo $Use_Id[$index];
                        foreach($Use_Name as $index => $Item){
                          ?>
                          ...
                          <tbody>
                                <tr id="member_template" >
                                  ...
                                  <td>
                                   <?php echo Format::htmlchars($Item) ?>
                                  </td>
                                  <td>
                                    <label>
                                      <input type="checkbox" data-name="member_alerts" value="0"   />
                                      <?php echo __('Alerts'); ?>
                                    </label>
                                    
                                    <a  class="pull-right drop-membershipUser" title="<?php echo __('Delete');
                                      ?>"><i  class="icon-trash"><input type="hidden" name="custId[]" value="<?php echo $Use_Id[$index];?>"></i></a>
                                  </td>
                                </tr>
                              </tbody>
...
?>
    ...
    <tbody>
      ...
      <tbody>
        <tr class="header">
            <td colspan="2">
                <?php //echo __('User Members'); ?>
                <div><small>
                <?php echo sprintf(__('Users who are members of %s'), __('this team')); ?>
                <i class="help-tip" href="#members"></i>
                </small></div>
            </td>
        </tr>
      <tr id="add_member_user">
        <td colspan="2">
          <i class="icon-plus-sign"></i>
          <select id="add_access_user" data-quick-add="user">
            <option value="0">&mdash; <?php echo __('Select User');?> &mdash;</option>
            <?php
            foreach ($users as $id=>$name) {
              echo sprintf('<option value="%d">%s</option>',$name[0],Format::htmlchars($name[1]));
            }
            ?>
          </select>
          <button type="button" class="action-button">
    ...
    <tbody>
      <tr id="member_template_user" class="hidden">
        <td>
          <input type="hidden" data-name="members_user[]" value="" />
        </td>
        <td>
          <label>
            <input type="checkbox" data-name="member_alerts" value="1" />
...
var addMemberUser = function(staffid, name, alerts, error) {
  if (!staffid) return;
  var copy = $('#member_template_user').clone();

  copy.find('[data-name=members\\[\\]]')
    .attr('name', 'members_user[]')
    .val(staffid);
  copy.find('[data-name^=member_alerts]')
    .attr('name', 'member_alerts['+staffid+']')
    .prop('checked', alerts);
...
$('#add_member_user').find('button').on('click', function() {
  var selected = $('#add_access_user').find(':selected'),
      id = parseInt(selected.val());
  if (!id)
    return;
addMemberUser(id, selected.text(), true);
  if ($('#team-lead-select option[value='+id+']').length === 0) {
    $('#team-lead-select').find('option[data-quick-add]')
    .before(
      $('<option>').val(selected.val()).text(selected.text())
...
$(document).on('click', 'a.drop-membershipUser', function() {
  var tr = $(this).closest('tr');
  //     id = tr.find('input[name^=members][type=hidden]').val();
  //     alert(id);
  // $('#add_access').append(
...
<?php if(isset($_SESSION["team_page_number"])){
?>
<button type="button"><a href="teams.php?sort=&order=&p=<?php echo $_SESSION["team_page_number"]; ?>">Back</a></button>
<?php
}
else{
  ?>
  <button type="button"><a href="teams.php">Back</a></button>
  <?php
}
